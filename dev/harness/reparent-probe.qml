...
// Can another plugin's panel content be taken out of its own window and drawn
// inside ours, with its bindings and behaviour intact?
//
...
//
// It prints STAGE1 with an empty black host window up, moves the content in,
// then prints what the moved tree says about itself. Screenshot the top-left
// 460x420 on each side of the move and compare: empty is one colour, after the
// move it is the audio panel.
//
// Measured 2026-09-18: empty host 0 non-black pixels of 193 200 and exactly one
// colour; after the move 12 013 non-black pixels and 1 546 colours; the moved
// tree reported live service data ("STEADY GROOVE", "55%") and re-laid out from
// 460 to 640 px when the host was resized. The plugin's own window never
// mapped.
ShellRoot {
  id: harness
