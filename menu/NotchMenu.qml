...
// Omarchy's menu (shell/plugins/menu/Menu.qml, 4.0.3), forked to draw inside
// the notch: the notch grows around this Item -- same surface, top edge on the
// screen edge -- instead of the menu opening its own full-screen window with a
// scrim and a bordered card. The verbatim upstream file is kept in
// dev/upstream/menu/ so upstream changes can be diffed and ported; this fork
// keeps upstream's structure and names on purpose.
//
...
//   - closing (a picked row, Escape, a click outside) emits closeRequested so
//     the notch shrinks back; the notch calls open()/close() like the host does
//   - the app library is the shell's when the facade offers one, otherwise
//     the menu's own instance of the shell's AppLibrary service, loaded the
...
Item {
  ...
  // How big the notch lets the menu get.
  ...
  // Test harnesses: the menu holds the keyboard while it is open, so a key
  // typed at the wrong moment must never run anything from a test notch.
  readonly property bool dryRun: Quickshell.env("NOTCH_MENU_DRY_RUN") === "1"
  property string lastAction: ""

  // For the notch's report.
  readonly property int rowCount: displayModel.count
  readonly property bool keysFocused: keyCatcher.activeFocus
  function rowLabels() {
    var out = []
    for (var i = 0; i < displayModel.count; i++) out.push(displayModel.get(i).label)
    return out
  }

  // The notch calls open(payloadJson) to show the menu and close() to hide it.
  ...
  // A bar plugin's facade has none, so the menu loads its own instance of the
  // shell's AppLibrary the first time it opens (by path: a test harness
  // without the shell's services still loads the menu).
  readonly property var appLibrary: root.shell && root.shell.appLibrary ? root.shell.appLibrary : ownAppLibrary.item
  readonly property var ownLibrary: ownAppLibrary.item
  property bool everOpened: false
  Loader {
    id: ownAppLibrary
    active: root.everOpened && !(root.shell && root.shell.appLibrary)
    source: Quickshell.shellPath("services/AppLibrary.qml")
  }
  property bool deleteConfirmOpen: false
  property var deleteTarget: null
  onOpenedChanged: {
    if (opened) { everOpened = true; return }
  ...
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font") ? Style.space(520) : Style.space(300)), root.maxWidth)
  property int visibleRowsHeight: root.dmenuActive ? dmenuRowListHeight(layoutSerial, displayModel.count, filterText) : rowListHeight(layoutSerial, displayModel.count, filterText, searchDivider)
  property int cardHeight: root.dmenuActive
    ? Math.min(contentMargin * 2 + headerHeight + (mode === "input" ? 0 : contentSpacing + visibleRowsHeight), root.maxHeight)
    : Math.min(contentMargin * 2 + headerHeight + contentSpacing + visibleRowsHeight, root.maxHeight)

  // Upstream freezes the card's top and its rows' height on the first search
  // keystroke or submenu move, so the card stops re-centring. The notch's top
  // is always the screen edge; only the rows' ceiling is frozen.
  property int rowsCeiling: -1
  function freezeRowsCeiling() {
  ...
  // Height the card can devote to rows before outgrowing the notch.
  function availableRowsHeight() {
    var available = root.maxHeight - root.contentMargin * 2 - root.headerHeight - root.contentSpacing
    // The starting menu sets the ceiling: drilling into a longer submenu
    // scrolls behind the fold instead of growing the card.
    if (root.rowsCeiling >= 0) available = Math.min(available, root.rowsCeiling)
    return available
  }
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "menu" || row.kind === "link") {
      ...
      opened = false
      filterText = ""
      root.lastAction = "launch " + appId
      if (root.appLibrary && !root.dryRun) root.appLibrary.launch(appId, label)
    } else {
  ...
  function confirmDelete() {
    ...
    if (!target) return
    root.cancel()
    root.lastAction = "remove " + target.appId
    if (root.appLibrary && !root.dryRun) root.appLibrary.remove(target.appId, target.label)
  }
  ...
  // The card. Upstream's full-screen window, scrim and bordered card are
  // gone: the notch is the surface, and a click outside it closes the menu
  // through the notch's focus grab.
  Item {
    id: card
    anchors.fill: parent
    visible: root.rowsLoaded
    readonly property real contentTopInset: root.contentMargin
    readonly property real contentRightInset: root.contentMargin
    readonly property real contentBottomInset: root.contentMargin
    readonly property real contentLeftInset: root.contentMargin
